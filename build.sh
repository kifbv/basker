...
# Append footer using the same template as other pages
sed "s/__SITE_NAME__/$SITE_NAME/g" "$FOOTER_TEMPLATE" >> "$index_file"

# Copy CSS files to the output directory
cp "$LIGHT_THEME" "$DARK_THEME" "$OUT_DIR"/

# Process the error page to replace placeholders with site name and footer template
sed -e "s/__SITE_NAME__/$SITE_NAME/g" -e "/__FOOTER_TEMPLATE__/r $FOOTER_TEMPLATE" -e "s/__FOOTER_TEMPLATE__//g" "error.html" > "$OUT_DIR/error.html"
